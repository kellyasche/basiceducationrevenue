...
The table below provides each of the revenue streams per APU for each school district.
...
cat.apu.districts <- original %>%
  ...
  mutate(revenue = ifelse(is.na(revenue), 0, revenue),
         revenue.apu = revenue / awadm22) %>%
  select(`District Name`, categories, revenue.apu) %>%
  group_by(`District Name`) %>%
  mutate(id = seq()) %>%
  ungroup() %>%
  left_join(var.codes, by = c("categories" = "Code")) %>%
  select(`District Name`, Category, revenue.apu, id) %>%
...
cat.apu.group <- original %>%
  ...
  ungroup() %>%
  group_by(group) %>%
  mutate(revenue.awadm22 = round(revenue / revenue[categories == "awadm22"])) %>%
  ungroup() %>%
  select(group, categories, revenue.awadm22) %>%
  filter(categories != "awadm22") %>%
  left_join(var.codes, by = c("categories" = "Code")) %>%
  select(group, revenue.awadm22, Category) %>%
  spread(Category, revenue.awadm22) %>%
  left_join(group.codes, by = c("group" = "Code")) %>%
  select(-group) %>%
  select(Definition, 1:15) 

datatable(cat.apu.group, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:13)),
                         scrollX = TRUE)) %>%
...
cat.apu.ruca <- original %>%
  ...
  mutate(revenue.awadm22 = round(revenue / revenue[categories == "awadm22"])) %>%
  ungroup() %>%
  select(Dem_Desc, categories, revenue.awadm22) %>%
  filter(categories != "awadm22") %>%
  left_join(var.codes, by = c("categories" = "Code")) %>%
  select(Dem_Desc, revenue.awadm22, Category) %>%
  spread(Category, revenue.awadm22) 
...
cat.apu.pr <- original %>%
  ...
  mutate(revenue.awadm22 = round(revenue / revenue[categories == "awadm22"])) %>%
  ungroup() %>%
  select(planning.region, categories, revenue.awadm22) %>%
  filter(categories != "awadm22") %>%
  left_join(var.codes, by = c("categories" = "Code")) %>%
  select(planning.region, revenue.awadm22, Category) %>%
  spread(Category, revenue.awadm22) 
...
cat.apu.edr <- original %>%
  ...
  mutate(revenue.awadm22 = round(revenue / revenue[categories == "awadm22"])) %>%
  ungroup() %>%
  select(edr, categories, revenue.awadm22) %>%
  filter(categories != "awadm22") %>%
  left_join(var.codes, by = c("categories" = "Code")) %>%
  select(edr, revenue.awadm22, Category) %>%
  spread(Category, revenue.awadm22) 
