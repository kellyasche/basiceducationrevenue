...
```{r unlinked components pct remaining total rev distrit map}
...
unlinked.pct.remaining.rev.districts.rank.1 <- unlinked.pct.remaining.rev.districts %>%
  ...
  mutate(unlinked.pct.total.rev.bins = cut(unlinked.pct.total.rev,
                                           breaks = c(unname(quantile(seq(0,1,.2)))),
                                           labels = c(1,2,3,4))) %>%
  left_join(school_districts[,c(3,4,12)], by = c("number", "type"))

names(unlinked.pct.remaining.rev.districts)

unlinked.pct.remaining.rev.districts.comp.map <- ggplot(filter(unlinked.pct.remaining.rev.districts.rank.1, unlinked.rank == 1)) +
  ...
  labs(title = "Unlinked component with highest percentage of total revenue", subtitle = "Local optional aid and small schools revenue become important to rural districts with low property values") +
  theme(legend.box.margin = margin(50, 0, 0, -100),
        text = element_text(size = 18))


unlinked.pct.remaining.rev.districts.pct.map <- ggplot(filter(unlinked.pct.remaining.rev.districts.rank.1, unlinked.rank == 1)) +
...
girafe(ggobj = unlinked.pct.remaining.rev.districts.pct.map, height_svg = 10, width_svg = 10) %>%
  girafe_options(opts_sizing(rescale = FALSE))
