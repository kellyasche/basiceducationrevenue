...
Let's look at only the districts that fall into the top 25% of boosts or difference. 

<br>

```{r top boosts diff only}
top.25pct.only.summary.districts <- top.25pct.summary %>%
  select(combo, n, Basic.cat, Entire.cat, Diff.cat) %>%
  filter(combo != "None") %>%
  gather(key = "category", value = "confirmation", 3:5) %>%
  filter(confirmation == "Yes") %>%
  select(combo, n, category) %>%
  add_row(combo = "Basic-diff", n = 0, category = "Basic.cat") %>%
  add_row(combo = "Basic-diff", n = 0, category = "Diff.cat") %>%
  group_by(category) %>%
  mutate(pct = n / sum(n)) %>%
  ungroup() %>%
  mutate(data_id = seq(n())) %>%
  mutate(definition = ifelse(combo == "Basic ed only", "Basic only",
                             ifelse(combo == "")))

names(top.25pct.only.summary.districts)

top.25pct.only.summary.districts.plot <- ggplot(top.25pct.only.summary.districts, aes(category, pct, fill = combo, group = combo)) +
  geom_col_interactive(aes(data_id = data_id, tooltip = paste(category, "\nCombination: ", combo, "\nNumber of districts with this combo: ", n, "\nPercent of districts with this combo: ", percent(pct, accuracy = .1), sep = ""))) +
  labs(x="", y = "", color="", title = "Top 25% combination within scenario")+
  scale_y_continuous(labels=scales::percent)+
  theme_bar+
...
girafe(ggobj = top.25pct.only.summary.districts.plot, width_svg = 10, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))    
