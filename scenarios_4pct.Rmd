...
# Comparing scenarios by percent increase to original revenue{.tabset}
...
```{r comparing change between scenarios pr}
comp.total.rev.change.pr <- master.4pct %>%
  left_join(master.bsrevonly.4pct, by = c("District Name", "number", "type", "categories")) %>%
  left_join(master.original, by = c("District Name", "number", "type", "categories")) %>%
  left_join(district.join, by = c("District Name", "number", "type")) %>%
...
datatable(comp.total.rev.change.pr, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:9)),
                         scrollX = TRUE)) %>%
  formatCurrency(2:5, "$") %>%
  formatPercentage(6, digits = 2) %>%
...
```{r comparing change between scenarios edr}
comp.total.rev.change.edr <- master.4pct %>%
  left_join(master.bsrevonly.4pct, by = c("District Name", "number", "type", "categories")) %>%
  left_join(master.original, by = c("District Name", "number", "type", "categories")) %>%
  left_join(district.join, by = c("District Name", "number", "type")) %>%
  group_by(edr) %>%
...
datatable(comp.total.rev.change.edr, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:9)),
                         scrollX = TRUE)) %>%
  formatCurrency(2:5, "$") %>%
  formatPercentage(6, digits = 2) %>%
...
Next, let's see what the percent change from original looks like at the county level.

The maps below show that there are a few counties that end up changing. 

<br>

```{r comparing change between scenarios county}
comp.total.rev.change.county <- master.4pct %>%
  left_join(master.bsrevonly.4pct, by = c("District Name", "number", "type", "categories")) %>%
  left_join(master.original, by = c("District Name", "number", "type", "categories")) %>%
  left_join(district.join, by = c("District Name", "number", "type")) %>%
...
comp.total.rev.change.county.map.1 <- ggplot(comp.total.rev.change.county) +
  geom_sf_interactive(color = "grey85", aes(geometry = geometry, fill = pct.diff.original.4pct.all, data_id = countyfp, tooltip = paste(`County Name`, "\nOriginal total revenue: ", dollar(total.rev.original), "\nTotal revenue 4% increase entire formula: ", dollar(total.rev.4pct.all), "\nPercent difference from original: ", percent(pct.diff.original.4pct.all, accuracy = .1), sep = ""))) +
  theme_sf+
  scale_fill_fermenter(palette = "PuBu", direction = 1, labels = scales::percent) +
  labs(subtitle = "4% increase to entire formula") +
  theme(legend.box.margin = margin(50, 0, 0, -100),
        text = element_text(size = 18))

comp.total.rev.change.county.map.2 <- ggplot(comp.total.rev.change.county) +
  geom_sf_interactive(color = "grey85", aes(geometry = geometry, fill = pct.diff.original.4pct.bsrev, data_id = countyfp, tooltip = paste(`County Name`, "\nOriginal total revenue: ", dollar(total.rev.original), "\nTotal revenue 4% increase bsrev only: ", dollar(total.rev.4pct.bsrev), "\nPercent difference from original: ", percent(pct.diff.original.4pct.bsrev, accuracy = .1), sep = ""))) +
  theme_sf+
  scale_fill_fermenter(palette = "PuBu", direction = 1, labels = scales::percent) +
  labs(subtitle = "4% increase to bsrev only") +
  theme(legend.box.margin = margin(50, 0, 0, -100),
        text = element_text(size = 18))

plot.row.1 <- plot_grid(comp.total.rev.change.county.map.1, comp.total.rev.change.county.map.2) 
  

title <- ggdraw() +
  draw_label(
    "Percent change from original revenue",
    fontface = "bold",
    x = 0,
    hjust = 0,
    size = 18
...
girafe(ggobj = plot_grid(title, plot.row.1,
                         ncol = 1, rel_heights = c(.1, 1)), height_svg = 10, width_svg = 10) %>%
...
# Comparing scenarios by APU{.tabset}

Let's see what this looks like at an APU basis.

Let's start by looking at school districts. When comparing the rankings, it doesn't seem like there's a ton of movement in terms of apu. 

The tables below show that not a single ranking changes in terms of revenue per APU for each of the scenarios across RUCA categories and regions.

<br>

```{r APU join}
awadm22.districts <- original %>%
  select(`District Name`, number, type, awadm22)

awadm22.group <- original %>%
  group_by(group) %>%
  summarize(awadm22 = sum(awadm22, na.rm = TRUE)) %>%
  ungroup() %>%
  left_join(group.codes, by = c("group" = "Code")) %>%
  select(Definition, awadm22)

awadm22.ruca <- original %>%
  group_by(Dem_Desc) %>%
  summarize(awadm22 = sum(awadm22, na.rm = TRUE)) %>%
  ungroup()
...
awadm22.edr <- original %>%
  group_by(edr) %>%
  summarize(awadm22 = sum(awadm22, na.rm = TRUE)) %>%
  ungroup()
```

```{r scenario comparison by APU districts}
comp.apu.districts <- comp.total.rev.change.districts %>%
  left_join(awadm22.districts, by = c("District Name", "number", "type")) %>%
  filter(awadm22 > 0) %>%
  mutate(total.rev.original.apu = total.rev.original / awadm22,
         total.rev.4pct.all.apu = total.rev.4pct.all / awadm22,
         total.rev.4pct.bsrev.apu = total.rev.4pct.bsrev / awadm22) %>%
  select(`District Name`, number, type, total.rev.original.apu, total.rev.4pct.all.apu, total.rev.4pct.bsrev.apu) %>%
  arrange(desc(total.rev.original.apu)) %>%
  mutate(rank.original = seq(n())) %>%
  arrange(desc(total.rev.4pct.all.apu)) %>%
  mutate(rank.4pct.all = seq(n())) %>%
  arrange(desc(total.rev.4pct.bsrev.apu)) %>%
  mutate(rank.4pct.bsrev = seq(n()))

datatable(comp.apu.districts, class = "cell-border stripe", filter = "top", rownames = FALSE,
...
Let's see what it looks like by public vs. charter schools. The table below shows no change in terms of funding per APU rank across all three scenarios. And, there is little difference between the 4% increase scenarios.

<br>

```{r scenario comparison by APU group}
comp.apu.group <- comp.total.rev.change.group %>%
  left_join(awadm22.group, by = c("Definition")) %>%
  filter(awadm22 > 0) %>%
  mutate(total.rev.original.apu = total.rev.original / awadm22,
         total.rev.4pct.all.apu = total.rev.4pct.all / awadm22,
         total.rev.4pct.bsrev.apu = total.rev.4pct.bsrev / awadm22) %>%
  select(Definition, total.rev.original.apu, total.rev.4pct.all.apu, total.rev.4pct.bsrev.apu) %>%
  arrange(desc(total.rev.original.apu)) %>%
  mutate(rank.original = seq(n())) %>%
  arrange(desc(total.rev.4pct.all.apu)) %>%
  mutate(rank.4pct.all = seq(n())) %>%
...
datatable(comp.apu.group, class = "cell-border stripe", filter = "top", rownames = FALSE,
...
Next, let's see if there are significant differences if grouped by RUCA categories and geographies.
...
```{r scenario comparison by APU ruca}
comp.apu.ruca <- comp.total.rev.change.ruca %>%
  left_join(awadm22.ruca, by = c("Dem_Desc")) %>%
  filter(awadm22 > 0) %>%
  mutate(total.rev.original.apu = total.rev.original / awadm22,
         total.rev.4pct.all.apu = total.rev.4pct.all / awadm22,
         total.rev.4pct.bsrev.apu = total.rev.4pct.bsrev / awadm22) %>%
  select(Dem_Desc, total.rev.original.apu, total.rev.4pct.all.apu, total.rev.4pct.bsrev.apu) %>%
  arrange(desc(total.rev.original.apu)) %>%
  mutate(rank.original = seq(n())) %>%
  arrange(desc(total.rev.4pct.all.apu)) %>%
  mutate(rank.4pct.all = seq(n())) %>%
...
datatable(comp.apu.ruca, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:6)),
                         scrollX = TRUE)) %>%
  formatCurrency(2:4, "$") 
```
...
```{r scenario comparison by APU pr}
comp.apu.pr <- comp.total.rev.change.pr %>%
  left_join(awadm22.pr, by = c("planning.region")) %>%
  filter(awadm22 > 0) %>%
  mutate(total.rev.original.apu = total.rev.original / awadm22,
         total.rev.4pct.all.apu = total.rev.4pct.all / awadm22,
         total.rev.4pct.bsrev.apu = total.rev.4pct.bsrev / awadm22) %>%
  select(planning.region, total.rev.original.apu, total.rev.4pct.all.apu, total.rev.4pct.bsrev.apu) %>%
  arrange(desc(total.rev.original.apu)) %>%
  mutate(rank.original = seq(n())) %>%
  arrange(desc(total.rev.4pct.all.apu)) %>%
  mutate(rank.4pct.all = seq(n())) %>%
...
datatable(comp.apu.pr, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:6)),
                         scrollX = TRUE)) %>%
  formatCurrency(2:4, "$") 
```
...
```{r scenario comparison by APU edr}
comp.apu.edr <- comp.total.rev.change.edr %>%
  left_join(awadm22.edr, by = c("edr")) %>%
  filter(awadm22 > 0) %>%
  mutate(total.rev.original.apu = total.rev.original / awadm22,
         total.rev.4pct.all.apu = total.rev.4pct.all / awadm22,
         total.rev.4pct.bsrev.apu = total.rev.4pct.bsrev / awadm22) %>%
  select(edr, total.rev.original.apu, total.rev.4pct.all.apu, total.rev.4pct.bsrev.apu) %>%
  arrange(desc(total.rev.original.apu)) %>%
  mutate(rank.original = seq(n())) %>%
  arrange(desc(total.rev.4pct.all.apu)) %>%
  mutate(rank.4pct.all = seq(n())) %>%
...
datatable(comp.apu.edr, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:6)),
                         scrollX = TRUE)) %>%
  formatCurrency(2:4, "$") 
```
