...
The table below shows that in all the scenarios public districts are significantly more likely to receive a top 25% boost compared to charter districts.
...
```{r top 25% related by group}
top.25pct.related <- top.25pct.boost.districts %>%
  left_join(district.join, by = c("District Name", "number", "type"))

top.25pct.related.group <- top.25pct.related %>%
  select(group, top.25pct.bsrevonly, top.25pct.entire.formula, top.25pct.diff) %>%
  gather(key = "scenario", value = "top.25pct.increase", 2:4) %>%
  filter(top.25pct.increase == "Yes") %>%
  group_by(group, scenario) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  complete(group, scenario, fill = list(n = 0)) %>%
  group_by(scenario) %>%
  mutate(pct.n = n / sum(n)) %>%
  ungroup() %>%
  left_join(group.codes, by = c("group" = "Code")) %>%
  select(Definition, scenario, n, pct.n) %>%
  left_join(prop.group, by = "Definition") %>%
  arrange(scenario)

names(top.25pct.related.group)

datatable(top.25pct.related.group, class = "cell-border stripe", filter = "top", rownames = FALSE,
...
```{r top 25% related by ruca}

top.25pct.related.ruca <- top.25pct.related %>%
  select(Dem_Desc, top.25pct.bsrevonly, top.25pct.entire.formula, top.25pct.diff) %>%
  gather(key = "scenario", value = "top.25pct.increase", 2:4) %>%
  filter(top.25pct.increase == "Yes") %>%
  group_by(Dem_Desc, scenario) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  complete(Dem_Desc, scenario, fill = list(n = 0)) %>%
  group_by(scenario) %>%
  mutate(pct.n = n / sum(n)) %>%
  ungroup() %>%
...
datatable(top.25pct.related.ruca, class = "cell-border stripe", filter = "top", rownames = FALSE,
          options = list(columnDefs = list(list(className = "dt-center", targets = 1:5)),
                         scrollX = TRUE)) %>%
  formatPercentage(4, digits = 2) %>%
  formatPercentage(6, digits = 2)
