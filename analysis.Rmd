...
What's interesting is that if you sort by `Rank of pct lost between  scenarios` you will see that the top three school districts would loss 1% 
...
Let's take a look to see if those schools that experience the largest "boost" from a 4% increase to basic education revenue component only to a 4% increase across the entire formula are related to whether it's a charter vs. public school, it's RUCA category, and it geographic location. We will use the top 25 school districts with the largest difference between the two scenarios.

The public vs. charter school table shows that there were no charter schools in the top 25. Obviously, this means that there is a statistically significant difference since charter schools make up 36% of the entire dataset. All 25 schoool districts with the largest difference between the two scenarios are public schools.

The RUCA categories also indicate a statistically significant difference between being in the top 25 and it's proportion of the total dataset. The biggest difference is the number of schools located in a town/rural mix county that are in the top 25 -  n = 13, pct = 52% compared to being only 25% of the entire dataset. The other major difference is how few school districts located in entirely urban counties are in the top 25 compared to it's proportion of the dataset - 8% vs 41% of the dataset.

The planning regions indicate that Northwest, Southwest, and Southeast make up a significantly larger proportion of the top 25 compared to it's population; 36% vs. 19%, 24% vs. 15%, and 28% vs. 11%, respectively. The other regions have significantly lower proportions of the top 25.

In addition, the EDR's show that the following regions have higher than expected proportions in the top 25;

* EDR 10 - Southeast: 28% vs. 11%
* EDR 4 - West Central: 16% vs. 6%
* EDR 5 - North Central: 16% vs. 5%
* EDR 8 - Southwest: 12% vs. 6%
* EDR 9 - South Central: 12% vs. 7%

The EDR's that had significantly lower than expected proportions in the top 25 were;

* EDR 11 - 7-county metro: 4% vs. 35%

Lastly, the county map shows that the location of schools that are in the top 25 are scattered throughout Greater Minnesota. 
...
top.boost.districts <- comp.total.rev.change.districts %>%
  filter(`Rank of pct gained between 2 scenarios` < 26) %>%
  left_join(district.join, by = c("District Name", "number", "type")) %>%
  left_join(group.codes, by = c("group" = "Code")) %>%
  filter(group %in% c(1,4)) 
...
top.boost.county.map <- ggplot(top.boost.county) +
  ...
  theme_sf+
  scale_fill_manual(values = brewer.pal(n = 3, "PuBu")) +
...
The range from above shows that about half of the school districts receive between .5% and 1% more by increasing the entire formula by 4% compared to only increasing the basic education revenue component. In addition, there does seem to be a relatioship between districts that receive the highest boost going from a 4% to basic education only vs.a 4% increase to the entire formula. So, what components are driving that difference?
...
### Top 25 districts with largest pct difference

<br>

```{r table driver of difference between total revenues top 20 districts}
driver.diff.total.rev.top.districts <- driver.diff.total.rev.districts %>%
  left_join(comp.total.rev.change.districts[,c(1,2,3,10)], by = c("District Name", "number", "type")) %>%
  filter(`Rank of pct gained between 2 scenarios` < 26) %>%
  arrange(`Rank of pct gained between 2 scenarios`) %>%
  filter(pct.diff > 0)
...
```{r chart driver of difference top 20 districts}
...
### Chart - categories strength of drive

<br>

```{r categories strength of drive}
cat.strength.top.25 <- driver.diff.total.rev.top.districts.n %>%
  mutate(strength.rank = ifelse(rank.pct.diff == 1, 3,
                           ifelse(rank.pct.diff == 2, 2, 1)),
         strength.multiplied.n = strength.rank * n) %>%
  group_by(categories) %>%
  summarize(total.n = sum(n),
            strength.multiplied.n = sum(strength.multiplied.n)) %>%
  ungroup() %>%
  mutate(strength = strength.multiplied.n / total.n)
  
names(cat.strength.top.25)
```
