...
Next, let's see if there are any relationships between the type of school, it's RUCA category, and it's region.
...
total.rev.group <- total.rev.districts %>%
  group_by(group) %>%
  summarize(total.rev.original = sum(Original),
            total.rev.bsrevonly = sum(`Basic education only`),
            total.rev.entire.formula = sum(`Entire formula`)) %>%
  ungroup()

names(total.rev.group)

comp.pct.diff.group <- master.5yr.scenarios.districts %>%
  select(`District Name`, number, type, categories, revenue.0yr, revenue.5yr, revenue.bsrevonly.5yr) %>%
  rename(revenue.original = revenue.0yr) %>%
  left_join(district.join, by = c("District Name", "number", "type")) %>%
  left_join(total.rev.districts[,c(1,2,3,7,5,6)], by = c("District Name", "number", "type")) %>%
  rename(total.rev.original = Original,
         total.rev.bsrevonly = `Basic education only`,
         total.rev.entire.formula = `Entire formula`) %>%
  group_by(group, categories) %>%
  summarize(awadm22 = sum(awadm22, na.rm = TRUE),
            revenue.original = sum(revenue.original, na.rm = TRUE),
            revenue.bsrevonly.5yr = sum(revenue.bsrevonly.5yr, na.rm = TRUE),
            revenue.5yr = sum(revenue.5yr, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(cat.rev.original.apu = revenue.original / awadm22,
         cat.rev.bsrevonly.apu = revenue.bsrevonly.5yr / awadm22,
         cat.rev.entire.formula.apu = revenue.5yr / awadm22) %>%
  select(group, categories, cat.rev.original.apu, cat.rev.bsrevonly.apu, cat.rev.entire.formula.apu) %>%
  mutate(diff.cat.bsrevonly.ef.apu = cat.rev.entire.formula.apu - cat.rev.bsrevonly.apu) 
