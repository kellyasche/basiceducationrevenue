...
The range from above shows that about half of the school districts receive between .5% and 1% more by increasing the entire formula by 4% compared to only increasing the basic education revenue component. What components are driving that difference?
...
### Top 20 districts with largest pct difference
...
driver.diff.total.rev.top.districts <- driver.diff.total.rev.districts %>%
  ...
  filter(`Rank of pct gained between 2 scenarios` < 21) %>%
  arrange(`Rank of pct gained between 2 scenarios`) %>%
  filter(pct.diff > 0)
  
names(driver.diff.total.rev.top.districts)
...
names(driver.diff.total.rev.top.districts.n)

driver.diff.total.rev.top.districts.plot <- ggplot(filter(driver.diff.total.rev.top.districts, rank.pct.diff < 4), aes(categories, rank.pct.diff, )) +
  geom_point_interactive(size = 5, aes(data_id = number, tooltip = paste(`District Name`, "\n", categories, "\nPercent this category is of difference between 2 scenarios: ", percent(pct.diff, accuracy = .1), sep = ""))) +
  labs(x="", y = "", color="", title = "Rank of categories driving difference between scenarios") +
  theme_line+
