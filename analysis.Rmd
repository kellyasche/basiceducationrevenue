...
names(change.total.rev.apu.summary.stats.districts)
...
ggplot(top.dist.diff.districts, aes(diff.between.scenarios)) +
...
top.25pct.summary <- top.25pct.boost.districts %>%
  ...
  gather(key = "category", value = "confirm.category", 4:6) %>%
  filter(confirm.category == "Yes") %>%
  add_row(combo = "Basic-diff", n = 0, pct = 0, category = "Basic.cat", confirm.category = "Yes") %>%
  add_row(combo = "Basic-diff", n = 0, pct = 0, category = "Diff.cat", confirm.category = "Yes")

names(top.25pct.summary)

top.25pct.summary.plot <- ggplot(top.25pct.summary, aes(category, pct, group = combo, fill = combo)) +
  geom_col_interactive(position = "dodge", aes(data_id = combo)) +
